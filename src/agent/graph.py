...
- `build_single_pass_graph` is acyclic and generates once. Evaluation runs use
  it, so their token and latency numbers stay comparable.
- `build_multi_agent_graph` adds the self-correction cycle.
...
def build_single_pass_graph() -> Any:
    """Acyclic graph: one generation, no retries."""
    ...
    workflow.add_conditional_edges(
        "code_validator",
        is_valid,
        {"execute": "code_executor", "reflect": "synthesizer"},
    )
...
def build_multi_agent_graph() -> Any:
    """Graph with the bounded self-correction cycle."""
    workflow = StateGraph(AgentState)
    _register_shared_nodes(workflow)
...
def run_agent(
    ...
    return graph.invoke(
        initial_state(query, session_id, dataset_ids),
        config=config,
    )
