...
class DatabaseManager:
    ...
    def __init__(self, settings: Optional[Settings] = None, in_memory: bool = False):
        ...
        self.in_memory = in_memory or (os.getenv("USE_IN_MEMORY_DB", "false").lower() in ("true", "1"))
        self._pg_pool = None
            ...
            if self._pg_pool:
                with conn.cursor() as cur:
                    cur.execute(
                        ...
                        """,
                        (
                            dataset.id, dataset.name, dataset.description, dataset.file_type, dataset.category,
                            dataset.blob_path, dataset.file_size_bytes, dataset.content_hash,
                            dataset.row_count, dataset.page_count, dataset.created_at, dataset.updated_at
                        ),
                    )
                    row = cur.fetchone()
                ...
                cur.execute(
                    ...
                    (
                        dataset.id, dataset.name, dataset.description, dataset.file_type, dataset.category,
                        dataset.blob_path, dataset.file_size_bytes, dataset.content_hash,
                        dataset.row_count, dataset.page_count,
            ...
            if self._pg_pool:
                with conn.cursor() as cur:
                    ...
                    if not row:
                        return None
                    return Dataset(
                        id=str(row[0]), name=row[1], description=row[2], file_type=row[3],
                        category=row[4], blob_path=row[5], file_size_bytes=row[6],
                        content_hash=row[7], row_count=row[8], page_count=row[9],
                ...
                return Dataset(
                    ...
                    blob_path=row["blob_path"], file_size_bytes=row["file_size_bytes"],
    ...
    def list_datasets(self, category: Optional[str] = None) -> List[Dataset]:
        ...
        with self.get_connection() as conn:
            if self._pg_pool:
                with conn.cursor() as cur:
                    if category:
                        cur.execute("SELECT * FROM datasets WHERE category = %s ORDER BY created_at DESC;", (category,))
                    else:
                ...
                cur = conn.cursor()
                if category:
                    cur.execute("SELECT * FROM datasets WHERE category = ? ORDER BY created_at DESC", (category,))
                else:
                    cur.execute("SELECT * FROM datasets ORDER BY created_at DESC")
                for row in cur.fetchall():
                    results.append(Dataset(
            ...
            if self._pg_pool:
                with conn.cursor() as cur:
                    cur.execute(
                        ...
                        """,
                        (
                            table_meta.id, table_meta.dataset_id, table_meta.table_name,
                            table_meta.display_name, table_meta.description, table_meta.row_count,
                            table_meta.column_count, table_meta.embedding, table_meta.created_at
                        ),
                    )
                    row = cur.fetchone()
                ...
                cur.execute(
                    ...
                    (
                        table_meta.id, table_meta.dataset_id, table_meta.table_name,
                        table_meta.display_name, table_meta.description, table_meta.row_count,
                        table_meta.column_count, emb_json, table_meta.created_at.isoformat()
                    ),
                )
                conn.commit()
            ...
            if self._pg_pool:
                with conn.cursor() as cur:
                    cur.execute(
                        ...
                        """,
                        (
                            col_meta.id, col_meta.table_id, col_meta.column_name, col_meta.data_type,
                            col_meta.is_primary_key, col_meta.is_foreign_key, col_meta.foreign_target_table,
                            col_meta.foreign_target_column, col_meta.null_percentage,
                            col_meta.distinct_values_count, json.dumps(col_meta.sample_values),
                            col_meta.description, col_meta.embedding, col_meta.created_at
                        ),
                    )
                    row = cur.fetchone()
                ...
                cur.execute(
                    ...
                    (
                        ...
                        col_meta.foreign_target_table, col_meta.foreign_target_column,
                        col_meta.null_percentage, col_meta.distinct_values_count,
                        json.dumps(col_meta.sample_values), col_meta.description,
                        emb_json, col_meta.created_at.isoformat()
                    ),
                )
                conn.commit()
                return col_meta.id
    ...
    def list_tables(self) -> List[TableMetadata]:
        ...
        with self.get_connection() as conn:
            if self._pg_pool:
                with conn.cursor() as cur:
                    cur.execute("SELECT id, dataset_id, table_name, display_name, description, row_count, column_count, created_at FROM table_metadata;")
                    for row in cur.fetchall():
                        tables.append(TableMetadata(
                            id=str(row[0]), dataset_id=str(row[1]), table_name=row[2],
                            display_name=row[3], description=row[4], row_count=row[5],
                            column_count=row[6], created_at=row[7]
                        ))
            else:
                cur = conn.cursor()
                ...
                for row in cur.fetchall():
                    tables.append(TableMetadata(
                        id=row["id"], dataset_id=row["dataset_id"], table_name=row["table_name"],
        ...
        with self.get_connection() as conn:
            if self._pg_pool:
                with conn.cursor() as cur:
                    ...
                    for row in cur.fetchall():
                        samples = json.loads(row[10]) if isinstance(row[10], str) else (row[10] or [])
                        columns.append(ColumnMetadata(
                            id=str(row[0]), table_id=str(row[1]), column_name=row[2],
                            data_type=row[3], is_primary_key=bool(row[4]), is_foreign_key=bool(row[5]),
                            foreign_target_table=row[6], foreign_target_column=row[7],
                            null_percentage=float(row[8] or 0.0), distinct_values_count=int(row[9] or 0),
                            sample_values=samples, description=row[11], created_at=row[12]
                        ))
            else:
                cur = conn.cursor()
                ...
                for row in cur.fetchall():
                    ...
                    columns.append(ColumnMetadata(
                        id=row["id"], table_id=row["table_id"], column_name=row["column_name"],
                        data_type=row["data_type"], is_primary_key=bool(row["is_primary_key"]),
                        ...
                        sample_values=samples, description=row["description"]
                    ))
        return columns
        ...
        with self.get_connection() as conn:
            if self._pg_pool:
                ...
                for row in rows:
                    ...
                    for kw in keywords:
                        if len(kw) > 2 and (kw in row["table_name"].lower() or kw in row["description"].lower()):
                            sim += 0.15
    ...
    def search_column_metadata_vectors(
        ...
        with self.get_connection() as conn:
            if self._pg_pool:
                ...
                cur.execute(f"SELECT * FROM column_metadata WHERE table_id IN ({placeholders})", table_ids)
                rows = cur.fetchall()
                scored_cols = []
                keywords = [k.lower() for k in re.findall(r"\w+", query_text)] if query_text else []
                for row in rows:
                    ...
                    sim = cosine_similarity(query_embedding, emb)
                    for kw in keywords:
                        if len(kw) > 2 and (kw in row["column_name"].lower() or kw in row["description"].lower()):
                            sim += 0.15
                    scored_cols.append((bool(row["is_primary_key"]), bool(row["is_foreign_key"]), sim, row))
                ...
                with conn.cursor() as cur:
                    for chunk in chunks:
                        cur.execute(
                            ...
                            """,
                            (
                                chunk.id, chunk.dataset_id, chunk.chunk_index, chunk.page_number,
                                chunk.section_title, chunk.content, chunk.token_count,
                                chunk.char_count, chunk.embedding, chunk.created_at
                            ),
                        )
            else:
                cur = conn.cursor()
                for chunk in chunks:
                    ...
                    cur.execute(
                        ...
                        (
                            chunk.id, chunk.dataset_id, chunk.chunk_index, chunk.page_number,
                            chunk.section_title, chunk.content, chunk.token_count,
                            chunk.char_count, emb_json, chunk.created_at.isoformat()
                        ),
                    )
                conn.commit()
        ...
        with self.get_connection() as conn:
            if self._pg_pool:
                with conn.cursor() as cur:
                    ...
                    """
                    cur.execute(sql, (
                        query_embedding, dataset_id, dataset_id,
                        query_text, query_text, dataset_id, dataset_id,
                        top_k
                    ))
                    rows = cur.fetchall()
                    return [
                ...
                sparse_ranks = {cid: rank + 1 for rank, (cid, _, _) in enumerate(sparse_scored[:50])}
    ...
    def create_dedicated_table(
        ...
        col_sqls = []
        if not pkey_col:
            col_sqls.append("_row_id INTEGER PRIMARY KEY AUTOINCREMENT" if self.in_memory else "_row_id BIGSERIAL PRIMARY KEY")
            ...
            if self._pg_pool:
                with conn.cursor() as cur:
                    cur.execute(
                        ...
                        """,
                        (
                            log_entry.id, log_entry.session_id, log_entry.query_text,
                            log_entry.engine, log_entry.status, log_entry.prompt_tokens,
                            log_entry.completion_tokens, log_entry.latency_ms,
                            log_entry.generated_code, log_entry.error_message, log_entry.created_at
                        ),
                    )
            else:
                cur = conn.cursor()
                cur.execute(
                    ...
                    (
                        ...
                        log_entry.engine, log_entry.status, log_entry.prompt_tokens,
                        log_entry.completion_tokens, log_entry.latency_ms,
                        log_entry.generated_code, log_entry.error_message,
