...
class TestTier3CrossFeatureCombinations:
    ...
    def test_comb_csv_upload_to_pruner_to_dedicated_db_to_synthesizer(self, sample_data_dir, blob_storage_dir, test_db, mock_embeddings, mock_llm):
        ...
        test_db.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (dataset_id, "sales.csv", "Sales transactions", "csv", "structured", str(blob_path), len(content), hashlib.sha256(content).hexdigest(), len(df))
        )
        ...
        test_db.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (table_id, dataset_id, "tbl_sales", "Sales Table", table_desc, len(df), len(df.columns), json.dumps(mock_embeddings.embed_text(table_desc)))
        )
        
        # 3. Two-Stage Schema Pruner
        query = "What is the total sales amount by region?"
        q_vec = mock_embeddings.embed_text(query)
        tables = test_db.fetchall("SELECT * FROM table_metadata WHERE dataset_id = ?", (dataset_id,))
        assert len(tables) == 1
    ...
    def test_comb_parquet_upload_to_pruner_to_duckdb_to_synthesizer(self, sample_data_dir, blob_storage_dir, test_db, mock_embeddings, mock_llm):
    ...
    def test_comb_excel_upload_to_pruner_to_pandas_sandbox_to_synthesizer(self, sample_data_dir, blob_storage_dir, mock_llm):
        ...
        sandbox_code = "result = df[df['stock'] < 50].sort_values('unit_cost', ascending=False).head(20)"
    ...
    def test_comb_unstructured_pdf_upload_to_hybrid_rag_to_synthesizer_with_citations(self, sample_data_dir, test_db, mock_embeddings, mock_llm):
        ...
        chunks = [
            ...
            ("Vector embeddings in PostgreSQL use HNSW indexing with cosine distance.", 2, "Section 2")
        ]
        for idx, (txt, page, sec) in enumerate(chunks):
            ...
            test_db.execute(
                ...
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (str(uuid.uuid4()), dataset_id, idx, page, sec, txt, len(txt.split()), len(txt), json.dumps(vec))
            )
            
        # Retrieval & Synthesis
        citations = ["[SecurityDoc.pdf, Page 1]", "[SecurityDoc.pdf, Page 2]"]
        synth = mock_llm.synthesize_answer("What is the token policy?", [{"text": chunks[0][0]}], citations=citations)
    ...
    def test_comb_multi_dataset_ingestion_and_benchmark_arena_comparison(self, sample_data_dir):
        ...
        # Strategy B (DuckDB)
        con = duckdb.connect(":memory:")
        res_b = con.execute(f"SELECT region, SUM(amount) as amount FROM read_csv_auto('{csv_path}') GROUP BY region ORDER BY region").df()
        con.close()
        
        # Strategy C (Pandas Sandbox)
        scope = {"df": df, "pd": pd}
        exec("result = df.groupby('region')['amount'].sum().reset_index().sort_values('region')", {}, scope)
        res_c = scope["result"]
    ...
    def test_comb_router_ambiguous_to_dataset_suggestion_to_clarification_flow(self, mock_llm, test_db):
        ...
        for name in ["sales_q3", "customer_churn", "financial_records"]:
            test_db.execute(
                "INSERT INTO datasets (id, name, description, file_type, category, blob_path, file_size_bytes, content_hash, row_count) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (str(uuid.uuid4()), name, f"Dataset for {name}", "csv", "structured", f"/blobs/{name}", 500, f"hash_{name}", 100)
            )
    ...
    def test_comb_router_structured_to_pandas_sandbox_with_ast_validation_and_trace(self, mock_llm, sample_data_dir):
    ...
    def test_comb_benchmark_arena_with_structured_equivalence_evaluation(self, sample_data_dir):
        ...
        # Golden SQL
        con = duckdb.connect(":memory:")
        golden_df = con.execute(f"SELECT region, COUNT(*) as cnt FROM read_csv_auto('{csv_path}') GROUP BY region ORDER BY region").df()
    ...
    def test_comb_schema_pruner_limit_20_propagated_to_all_three_engines(self, mock_llm):
        ...
        sql_a = mock_llm.generate_sql("List all orders", "CREATE TABLE tbl_sales (id INT);", dialect="postgres")
        sql_b = mock_llm.generate_sql("List all orders", "CREATE TABLE tbl_sales (id INT);", dialect="duckdb")
    ...
    def test_comb_dataset_deletion_cleans_blob_and_pgvector_and_metadata(self, test_db, tmp_path):
        ...
        test_db.execute(
            "INSERT INTO datasets (id, name, description, file_type, category, blob_path, file_size_bytes, content_hash, row_count) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (dataset_id, "delete_me.csv", "To be deleted", "csv", "structured", str(blob_file), 10, "del_hash", 1)
        )
        test_db.execute(
